...
bool getFile(String url, String filename, String TOKEN) {
  ...
  if ((WiFi.status() == WL_CONNECTED)) {
    ...
    http.begin(url);
    //if the image url might have aredirects
    http.setFollowRedirects(HTTPC_STRICT_FOLLOW_REDIRECTS);
    //token (musicbrainz api) for faster response
    ...
    if (httpCode == 200) {
      fs::File f = LittleFS.open(filename, "w+", false);
      if (!f) {
